# Перевод документации Instaparse 1.2.14

*А что если контекстно-свободные грамматики были бы проще регулярных выражений?*

## Ключевые особенности

Создатели Instaparse стремились создать простой инструмент для построения парсеров на Clojure.

+ Библиотека поддерживает две стандартных нотации для описания грамматик EBNF и ABNF. Парсер принимает на вход исходную строку а возвращает дерево разбора для этой строки.
+ Поддерживаются *левосторонние*, *правосторение* и *неоднозначные* граматики.
+ Библиотека расширяет возможности контекстно-свободных грамматик используя PEG-синтаксис с прямым и обратным просмотром входного потока символов.
+ Поддерживаются оба наиболее популярных форматов деревьев Clojure для использования в качестве выходных данных (hiccup и enlive).
+ Возврат подробного описания ошибок разбора входных данных.
+ При желании возможно получить ленивые последовательности в качестве выходных данных, что весьма полезно при отладки неоднозначных грамматик.
+ Режим "тщательного разбора", в котором остаточные фрагменты строк встраиваются в дерево разбора.
+ Дополнительная библиотека комбинатор для программного постороения грамматик.
+ Высокая производительность.

## Быстрый старт

Instaparse требует Clojure версии 1.5.1 либо более позднюю (скорее всего вам удастся запустить его с более ранними версиями clojure, однако скорость работы будет низкой).

Для добавления зависимости в leiningen вам необходимо добавить следующую строку в файл проекта:

[instaparse "1.2.14"]

Для подключения instaparse к вашему пространству имен вы можете использовать следующий код:

(ns example.core
  (:require [instaparse.core :as insta]))


### Создание вашего первого парсера

Ниже приведен типичный пример контекстно-свободной грамматики. Во многих учебниках символом `S` обозначается "корневое/стартовое" правило грамматики. Мы так-же не станим отступать от традиций:
